...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/baoyxing/go-tools/utils/wechatpay/auth/validators"
...
func (h *Handler) ParseNotifyRequest(
	...
	signType := requestContext.Request.Header.Get("Wechatpay-Signature-Type")
	if signType == "" {
	...
	if err := suite.validator.Validate(ctx, requestContext); err != nil {
		return nil, fmt.Errorf("invalid notification, err: %v, request: %+v",
			err, requestContext.Request.Body())
	}

	body, err := requestContext.Body()
	if err != nil {
		return nil, err
	}
